...
//    @Bulkhead(name = NAME_CIRCUIT, type = Bulkhead.Type.THREADPOOL)
    @TimeLimiter(name = NAME_CIRCUIT, fallbackMethod = "getAvailableProductsTimeLimiterFallback")
    public CompletableFuture<List<Product>> getProductsWithQuantityGreaterThanZero() {
//        System.out.println("Retry method called " + methodCallTimes++ + " times at " + new Date());
        List<Product> products = productService.getAllProduct();
        List<Product> productList = new ArrayList<>();
        for(Product product : products){
            final String url = "http://localhost:8084/inventory/getinventoryofproduct/" + product.getSkuCodeProduct();
